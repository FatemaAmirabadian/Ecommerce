...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({ message: "new user added to db" }, { status: 201 });
  } catch (err) {
    console.error(err);
    return NextResponse.json({ status: "failed", message: "Error in storing data in DB" }, { status: 500 });
  }
}
...
export async function PUT(request) {
  try {
    ...
    console.error(err);
    return NextResponse.json({ status: "failed", message: "Error in updating user" }, { status: 500 });
  }
}
